...
public:
    ...
    ALWAYS_INLINE RefPtr(T* ptr) : m_ptr(RefDerefBase::refIfNotNull(ptr)) { }
    ALWAYS_INLINE RefPtr(T& ptr) : m_ptr(&RefDerefBase::ref(ptr)) { }
    ALWAYS_INLINE RefPtr(const RefPtr& o) : m_ptr(RefDerefBase::refIfNotNull(PtrTraits::unwrap(o.m_ptr))) { }

    template<typename X, typename Y, template <typename> typename Z> RefPtr(const RefPtr<X, Y, Z>& o) : m_ptr(RefDerefBase::refIfNotNull(PtrTraits::unwrap(o.get()))) { }

    ALWAYS_INLINE RefPtr(RefPtr&& o) : m_ptr(o.leakRef()) { RefDerefBase::moveRef(o); }
    template<typename X, typename Y, template <typename> typename Z> RefPtr(RefPtr<X, Y, Z>&& o) : m_ptr(o.leakRef()) { RefDerefBase::moveRef(o); }
